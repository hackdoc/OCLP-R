...
from ...support import subprocess_wrapper, kdk_handler, translate_language
...
class KernelDebugKitMerge:
    ...
    def _matching_kdk_already_merged(self, kdk_path: str) -> bool:
        ...
        try:
            ...
            if oclp_plist_data["Kernel Debug Kit Used"] == str(kdk_path):
                logging.info(self.trans["- Matching KDK determined to already be merged, skipping"])
                return True
        except:
            pass
    ...
    def _backup_hid_cs(self) -> None:
        ...
        logging.info(self.trans["- Backing up IOHIDEventDriver CodeSignature"])
    ...
    def _restore_hid_cs(self) -> None:
        ...
        logging.info(self.trans["- Restoring IOHIDEventDriver CodeSignature"])
        cs_path = Path(self.mount_location) / Path("System/Library/Extensions/IOHIDFamily.kext/Contents/PlugIns/IOHIDEventDriver.kext/Contents/_CodeSignature")
        if not cs_path.exists():
            logging.info(self.trans["  - CodeSignature folder missing, creating"])
    ...
    def _merge_kdk(self, kdk_path: str) -> None:
        ...
        """
        logging.info(self.trans["- Merging KDK with Root Volume: {kdk_name}"].format(kdk_name=Path(kdk_path).name))
        ...
        if not (Path(self.mount_location) / Path("System/Library/Extensions/System.kext/PlugIns/Libkern.kext/Libkern")).exists():
            logging.info(self.trans["- Failed to merge KDK with Root Volume"])
            raise Exception(self.trans["Failed to install KDK"])
        logging.info(self.trans["- Successfully merged KDK with Root Volume"])
    ...
    def merge(self, save_hid_cs: bool = False) -> str:
        ...
        if kdk_obj.success is False:
            logging.info(self.trans["Unable to get KDK info: {error_msg}"].format(error_msg=kdk_obj.error_msg))
            raise Exception(self.trans["Unable to get KDK info: {error_msg}"].format(error_msg=kdk_obj.error_msg))
        ...
        if kdk_obj.kdk_already_installed is False:
            ...
            if not kdk_download_obj:
                logging.info(self.trans["Could not retrieve KDK: {error_msg}"].format(error_msg=kdk_obj.error_msg))
                raise Exception(self.trans["Could not retrieve KDK: {error_msg}"].format(error_msg=kdk_obj.error_msg))
            ...
            if kdk_download_obj.download_complete is False:
                ...
                logging.info(self.trans["Could not download KDK: {error_msg}"].format(error_msg=error_msg))
                raise Exception(self.trans["Could not download KDK: {error_msg}"].format(error_msg=error_msg))

            if kdk_obj.validate_kdk_checksum() is False:
                logging.info(self.trans["KDK checksum validation failed: {error_msg}"].format(error_msg=kdk_obj.error_msg))
                raise Exception(self.trans["KDK checksum validation failed: {error_msg}"].format(error_msg=kdk_obj.error_msg))
            ...
            if kdk_obj.kdk_already_installed is False:
                ...
                logging.warning(self.trans["KDK was not installed, but should have been: {error_msg}"].format(error_msg=kdk_obj.error_msg))
                raise Exception(self.trans["KDK was not installed, but should have been: {error_msg}"].format(error_msg=kdk_obj.error_msg))
