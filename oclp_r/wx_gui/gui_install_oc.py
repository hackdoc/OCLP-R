...
class InstallOCFrame(wx.Frame):
    ...
    def __init__(self, parent: wx.Frame, title: str, global_constants: constants.Constants, screen_location: tuple = None):
        self.trans = TranslateLanguage(global_constants).gui_install_oc()
        logging.info(self.trans["Initializing Install OpenCore Frame"])
    ...
    def _generate_elements(self) -> None:
        ...
        # Title label: Install OpenCore
        title_label = wx.StaticText(self, label=self.trans["Install OpenCore"], pos=(-1,5))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        title_label.Centre(wx.HORIZONTAL)

        # Text: Parsing local disks...
        text_label = wx.StaticText(self, label=self.trans["Fetching information on local disks..."], pos=(-1,30))
    ...
    def _display_disks(self) -> None:
        ...
        # Title label: Install OpenCore
        title_label = wx.StaticText(dialog, label=self.trans["Install OpenCore"], pos=(-1,5))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        title_label.Centre(wx.HORIZONTAL)

        # Text: select disk to install OpenCore onto
        text_label = wx.StaticText(dialog, label=self.trans["Select disk to install OpenCore onto:"], pos=(-1, title_label.GetPosition()[1] + title_label.GetSize()[1] + 5))
        text_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_NORMAL))
        text_label.Centre(wx.HORIZONTAL)

        # Add note: "Missing disks? Ensure they're FAT32 or formatted as GUID/GPT"
        gpt_note = wx.StaticText(dialog, label=self.trans["Missing disks? Ensure they're FAT32 or formatted as GUID/GPT"], pos=(-1, text_label.GetPosition()[1] + text_label.GetSize()[1] + 5))
        ...
        if self.available_disks:
            ...
            if disk_root:
                ...
                logging.info(f"{self.trans['Checking if booted disk is present:']} {disk_root}")
            ...
            spacer = 0
            logging.info(f"{self.trans['Available disks:']}")
            ...
            if disk_root:
                ...
                disk_label = wx.StaticText(dialog, label=self.trans["Note: Blue represents the disk OpenCore is currently booted from"], pos=(-1, disk_button.GetPosition()[1] + disk_button.GetSize()[1] + 5))
            ...
            disk_label = wx.StaticText(dialog, label=self.trans["Failed to find any applicable disks"], pos=(-1, gpt_note.GetPosition()[1] + gpt_note.GetSize()[1] + 15))
            disk_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_BOLD))
            disk_label.Centre(wx.HORIZONTAL)

        # Add button: Search for disks again
        search_button = wx.Button(dialog, label=self.trans["Search for disks again"], size=(150,30), pos=(-1, disk_label.GetPosition()[1] + disk_label.GetSize()[1] + 15))
        ...
        return_button = wx.Button(dialog, label=self.trans["Return to Main Menu"], size=(150,30), pos=(-1, search_button.GetPosition()[1] + 35))
    ...
    def _display_volumes(self, disk: str, dataset: dict) -> None:
        ...
        spacer = 0
        logging.info(f"{self.trans['Available partitions for ']}{disk}:")
        for partition in partitions:
            logging.info(f"- {partitions[partition]['partition']} - {partitions[partition]['name']} - {partitions[partition]['size']}")
            disk_button = wx.Button(dialog, label=f"{partitions[partition]['partition']} - {partitions[partition]['name']} - {partitions[partition]['size']}", size=(longest_label,30), pos=(-1, text_label.GetPosition()[1] + text_label.GetSize()[1] + 15 + spacer))
            disk_button.Centre(wx.HORIZONTAL)
        ...
        # Add button: Return to main menu
        return_button = wx.Button(dialog, label=self.trans["Return to Main Menu"], size=(150,30), pos=(-1, disk_button.GetPosition()[1] + disk_button.GetSize()[1]))
        return_button.Centre(wx.HORIZONTAL)
        return_button.Bind(wx.EVT_BUTTON, self.on_return_to_main_menu)
    ...
    def _install_oc_process(self, partition: dict) -> None:
        ...
        dialog = wx.Dialog(
            self,
            title=f"{self.trans['Installing OpenCore to ']} {partition}",
            style=wx.CAPTION | wx.CLOSE_BOX,
            size=(370, 200)
        )

        # Add text: "Installing OpenCore to {partition}"
        text_label = wx.StaticText(dialog, label=f"{self.trans['Installing OpenCore to ']} {partition}", pos=(-1, 10))
        text_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        text_label.Centre(wx.HORIZONTAL)
        ...
        # Add button: Return to main menu
        return_button = wx.Button(dialog, label=self.trans["Return to Main Menu"], size=(150,30), pos=(-1, text_box.GetPosition()[1] + text_box.GetSize()[1] + 10))
        return_button.Centre(wx.HORIZONTAL)
        return_button.Bind(wx.EVT_BUTTON, self.on_return_to_main_menu)
    ...
    def _invoke_install_oc(self, partition: dict) -> None:
        ...
        if self.result is True:
            if self.constants.update_stage != gui_support.AutoUpdateStages.INACTIVE and self.constants.detected_os >= os_data.os_data.big_sur:
                ...
                gui_support.RestartHost(self).restart(message=f"{self.trans['OpenCore has finished installing to disk.\n\nYou will need to reboot and hold the Option key and select OpenCore/Boot EFI\'s option.\n\nWould you like to reboot?']}")
            else:
                popup_message = wx.MessageDialog(
                    self,
                    f"{self.trans['OpenCore has finished installing to disk.\n\nYou can eject the drive, insert it into the ']} {self.constants.custom_model}, {self.trans['reboot, hold the Option key and select OpenCore/Boot EFI\'s option.']}", self.trans["Success"],
                    wx.OK
                )
                popup_message.ShowModal()
        else:
    ...
    def _install_oc(self, partition: dict) -> None:
        ...
        Install OpenCore to disk
        """
        logging.info(f"{self.trans['Installing OpenCore to ']} {partition}")
        ...
        try:
            ...
            logging.error(f"{self.trans['An internal error occurred while installing:\n']}")
