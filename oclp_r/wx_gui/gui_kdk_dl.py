...
class KDKDownloadFrame(wx.Frame):
    ...
    def _display_available_installers(self, event: wx.Event = None, show_full: bool = False) -> None:
        ...
        if installers:
            ...
            logging.info(f"Available installers on Intsant ({'All entries' if show_full else 'Latest only'}):")
            xnu_name={
                "26":"Tahoe Beta",
                "15":"Sequoia",
                "14":"Sonoma",
            ...
            wx.MessageDialog(self.frame_modal, "Failed to download KDKs Catalog from Intsant", "Error", wx.OK | wx.ICON_ERROR).ShowModal()
            self.on_return_to_main_menu()
        if show_full is False:
            self.list.Select(-1)
