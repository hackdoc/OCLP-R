...
class BuildFrame(wx.Frame):
    ...
    def __init__(self, parent: wx.Frame, title: str, global_constants: constants.Constants, screen_location: tuple = None) -> None:
        self.trans = TranslateLanguage(global_constants).gui_build()
        logging.info(self.trans["Initializing Build Frame"])
    ...
    def _generate_elements(self, frame: wx.Frame = None) -> None:
        ...
        title_label = wx.StaticText(frame, label=self.trans["Build and Install OpenCore"], pos=(-1,5))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        title_label.Centre(wx.HORIZONTAL)

        model_label = wx.StaticText(frame, label=f"{self.trans['Model:']} {self.constants.custom_model or self.constants.computer.real_model}", pos=(-1,30))
        model_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_NORMAL))
        model_label.Centre(wx.HORIZONTAL)

        # Button: Install OpenCore
        install_button = wx.Button(frame, label=self.trans["🔩 Install OpenCore"], pos=(-1, model_label.GetPosition()[1] + model_label.GetSize()[1]), size=(150, 30))
        ...
        # Button: Return to Main Menu
        return_button = wx.Button(frame, label=self.trans["Return to Main Menu"], pos=(-1, text_box.GetPosition()[1] + text_box.GetSize()[1] + 5), size=(150, 30))
    ...
    def _invoke_build(self) -> None:
        ...
        if self.build_successful is False:
            dialog = wx.MessageDialog(
                parent=self,
                message=self.trans["An error occurred while building OpenCore"],
                caption=self.trans["Error building OpenCore"],
        ...
        dialog = wx.MessageDialog(
            parent=self,
            message=f"{self.trans['Would you like to install OpenCore now?']}",
            caption=self.trans["Finished building your OpenCore configuration!"],
            style=wx.YES_NO | wx.ICON_QUESTION
        )
        dialog.SetYesNoLabels(self.trans["Install to disk"], self.trans["View build log"])
    ...
    def _build(self) -> None:
        ...
        try:
            ...
            logging.error(self.trans["An internal error occurred while building:\n"])
            ...
            if "TypeError: unsupported type: <class 'NoneType'>" in traceback.format_exc():
                logging.error(self.trans["If you continue to see this error, delete the following file and restart the application:"])
                logging.error(self.trans["Path: /Users/Shared/.com.hackdoc.oclp-r.plist"])
